...
{
    if (root->insert(value) != NULL)
    {
        InternalNode *newRoot = new InternalNode(internalSize, leafSize, NULL, NULL, NULL);//if node is emppty call insert
        newRoot->insert(root, root->getRightSibling());
        root = newRoot;
    }
} // BTree::insert()
